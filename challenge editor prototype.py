...
voucherlist = ["overstock_norm", "overstock_plus", "hone", "glow_up", "clearance_sale", "liquidation", "reroll_surplus", "reroll_glut", "crystal_ball", "omen_globe", "grabber",
...
header = ["testchallenge", "", "", "created using ValkyRiver's challenge editor prototype"] #steammodded header: name, id, author, description
customrules = []
...
def printmod(header, customrules, modifiers, jokers, consumables, vouchers, deck, decktype, bannedjokers, bannedconsumables, bannedvouchers, bannedpacks, bannedtags, bannedblinds):
    ...
    if len(consumables) == 0: #consumables
        ...
        for consumable in consumables:
            print(luaconsumable(consumable[0], consumable[1], consumable[2]))
        print("    },")
...
def export(header, customrules, modifiers, jokers, consumables, vouchers, deck, decktype, bannedjokers, bannedconsumables, bannedvouchers, bannedpacks, bannedtags, bannedblinds):
    ...
    if len(consumables) == 0: #consumables
        ...
        f.write("    consumeables = {\n")
        for consumable in consumables:
            f.write(luaconsumable(consumable[0], consumable[1], consumable[2])+"\n")
        f.write("    },\n")
    if len(vouchers) == 0: #vouchers
        f.write("    vouchers = {},\n")
    else:
